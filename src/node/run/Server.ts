...
export class Server {
  ...
  constructor(argv: any) {
    ...
    if (!path.isAbsolute(this._options.htmlDir)) {
      this._options.htmlDir = path.resolve(
        process.cwd(),
        this._options.htmlDir
      );
    }
  ...
  start() {
    this._app.use(
      "/",
      express.static(this._options.htmlDir, {
        index: false
      })
    );
    console.log("using html dir:", this._options.htmlDir);
    try {
      this._app.listen(this._options.port, this._options.listen, () => {
        console.log(
          "listen sucessed on:",
          this._options.listen,
          this._options.port
        );
      });
    } catch (e) {
